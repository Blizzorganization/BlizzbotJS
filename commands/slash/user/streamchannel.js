...
async function run(client, interaction) {
    if (!interaction.member.voice) {
        return interaction.reply({ content: "Du musst dich in einem Sprachkanal befinden um diesen Befehl ausführen zu können.", ephemeral: true });
    }
    const vc = interaction.member.voice.channel;
    if (!client.config.channels.voiceCategory.includes(vc.parentId)) {
        return interaction.reply({ content: "Um einen Streamchannel zu erzeugen musst du dich in einem Standard Voicechannel befinden.", ephemeral: true });
    }
    ...
    interaction.reply({ content: "Ein Streamchannel wurde erzeugt.", ephemeral: true });
}
