...
async function run(client, interaction) {
  const embed = new MessageEmbed()
    ...
    .addField(
      "/minecraftname [Name]",
      "Gibt deinen aktuellen Minecraft-Account wieder",
    )
  ...
  const ccmds = (await CustomCommand.findAll()).map(
    (c) => `${client.config.prefix}${c.commandName}`,
  );
  if (ccmds.length > 0)
    embed.addField("**__Temporäre Befehle:__**", ccmds.join(", "));
