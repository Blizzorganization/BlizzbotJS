...
async function handleCommands(client, message) {
  ...
  if (cmd) {
    ...
    const ccmd = await CustomCommand.findOne({
    ...
    if (ccmd) {
      ...
      if (alias.type === "ccmd") {
        const accmd = await CustomCommand.findOne({
          where: { commandName: alias.command },
        });
        if (accmd) {
...
function handleAdminCommands(client, message) {
  ...
  if (cmd) {
    if (
      cmd.perm <= permissions.dev &&
      message.member.roles.cache.has(client.config.roles.dev)
    )
      return cmd.run(client, message, args);
    if (message.author.id === message.guild.ownerId)
      return cmd.run(client, message, args);
...
export function checkMessage(client, message) {
  ...
  if (
    ...
    message.member.roles.cache
      .map((r) => r.id)
      .some((r) => client.config.roles.noFilter.includes(r))
  )
    return false;
  let links = [...message.content.matchAll(linkRegex)];
  if (message.channel.id === client.config.channels.clips)
    links = links.filter(
      (l) =>
        !l
          .toString()
  ...
  if (links.length > 0) {
    ...
    if (
      shouldUnverify &&
      message.member.roles.cache.has(client.config.roles.verify)
    )
      unverify(client, message.member);
    return true;
  }
  if (client.blacklist.length > 0) {
    const lowerMessage = message.content.toLowerCase();
    if (
      client.blacklist.some((blword) => lowerMessage.indexOf(blword) !== -1)
    ) {
      ...
      message.author.createDM().then((dmChannel) => {
        dmChannel.send(
          `Ihre Nachricht mit dem Inhalt **${message.content.slice(0, 1900)}** wurde entfernt. Melden Sie sich bei Fragen an einen Moderator.`,
        );
      });
      return true;
    }
...
export async function handle(client, message) {
  ...
  if (client.config.channels.commands.includes(message.channelId))
    return handleCommands(client, message);

  if (client.config.channels.modCommands.includes(message.channelId))
    return handleModCommands(client, message);

  if (client.config.channels.adminCommands.includes(message.channelId))
    return handleAdminCommands(client, message);
...
async function calculateExperience(message) {
  ...
  if (xpuser.username !== message.author.username)
    await xpuser.update({ username: message.author.username });
